    ...
    <div class="container">
        ...
        <div class="main-content">
            ...
            <div class="content" id="content-area">
                <?php
                $stmt = $conn->prepare("SELECT docuRoute3, student_id, route3_id FROM route3proposal_files WHERE adviser_id = ?");
                $stmt->bind_param("s", $adviser_id);
                $stmt->execute();
                $result = $stmt->get_result();
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        $filePath = htmlspecialchars($row['docuRoute3']);
                        $fileName = basename($filePath);
                        $student_id = htmlspecialchars($row['student_id']);
                        $route3_id = htmlspecialchars($row['route3_id']);
