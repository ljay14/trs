...
$stmt = $conn->prepare("SELECT student_id, route3_id FROM route3proposal_files WHERE adviser_id = ?");
if ($stmt === false) {
...
if ($result->num_rows > 0) {
    ...
    $route3_id = $row['route3_id']; // Now you have the route3_id
} else {
...
<?php
$stmt = $conn->prepare("SELECT docuRoute3, student_id, route3_id, group_number, controlNo, fullname, title FROM route3proposal_files WHERE adviser_id = ?");
$stmt->bind_param("s", $adviser_id);
$stmt->execute();
$result = $stmt->get_result();
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $route1_id = htmlspecialchars($row['route3_id'], ENT_QUOTES);
