...
function sendStudentNotificationEmail($student_email, $student_name, $adviser_name, $feedback_summary) {
    try {
        ...
        $email_data = [
            ...
            'body' => "<h2>New Feedback Submitted - Thesis Routing System</h2>
            <p>Dear $student_name,</p>
            <p>New feedback has been submitted by $adviser_name.</p>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['dateSubmitted'])) {
    ...
    if ($insertSuccess && !empty($student_email)) {
        if (isValidEmail($student_email)) {
            $emailSent = sendEmailInBackground($student_email, $studentName, $fullname, $feedback_summary);
            if ($emailSent) {
