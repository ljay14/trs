    ...
    <div class="container">
        ...
        <div class="main-content">
            ...
            <div class="content" id="content-area">
                <?php
                $stmt = $conn->prepare("SELECT docuRoute2, student_id, route2_id FROM route2final_files WHERE adviser_id = ?");
                $stmt->bind_param("s", $adviser_id);
                $stmt->execute();
                $result = $stmt->get_result();
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        $filePath = htmlspecialchars($row['docuRoute2']);
                        $fileName = basename($filePath);
                        $student_id = htmlspecialchars($row['student_id']);
                        $route2_id = htmlspecialchars($row['route2_id']);
