    ...
    try {
        ...
        $mail->Body = "
            <div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 5px;'>
                ...
                <p>A new final defense document (Route 1) has been submitted and requires your review.</p>
        ...
        ";
        $mail->AltBody = "Dear {$adviser_name}, A new final defense document (Route 1) has been submitted by {$fullname} with the title '{$title}' and requires your review. Please login at: {$login_url}";
    ...
    if ($result->num_rows > 0) {
        ...
        if (in_array($_FILES["docuRoute1"]["type"], $allowedTypes)) {
            ...
            if (move_uploaded_file($fileTmpPath, $newFilePath)) {
                ...
                $updateStmt = $conn->prepare("UPDATE route1final_files SET docuRoute1 = ? WHERE route1_id = ?");
        ...
        if (isset($_FILES["docuRoute1"]) && $_FILES["docuRoute1"]["error"] == UPLOAD_ERR_OK) {
            ...
            if (in_array($_FILES["docuRoute1"]["type"], $allowedTypes)) {
                ...
                if ($count > 0) {
                    ...
                    $stmt = $conn->prepare("INSERT INTO route1final_files (student_id, docuRoute1, department, panel1_id, panel2_id, panel3_id, panel4_id, panel5_id, adviser_id, date_submitted, controlNo, fullname, group_number, title, school_year) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                    if ($stmt) {
                            ...
                            if ($adviser_email) {
                                if (isValidEmail($adviser_email)) {
                                    ...
                                    if ($emailSent) {
                                        ...
                                        error_log("Success: Notification email sent to adviser ($adviser_email) for Final Route 1 file upload.");
                                    } else {
                                        ...
                                        error_log("Error: Failed to send notification email to adviser ($adviser_email) for Final Route 1 file upload.");
                                    }
                                } else {
                                    echo "<script>alert('File uploaded successfully but adviser email address is invalid.'); window.location.href = 'route1.php';</script>";
                                    error_log("Error: Invalid adviser email address ($adviser_email) for Final Route 1 file upload.");
                                }
                            } else {
                                ...
                                error_log("Warning: No adviser email available for notification during Final Route 1 file upload.");
                            }
                        } else {
            ...
            fetch(`get_all_forms.php?route1_id=${encodeURIComponent(route1_id)}`)
                ...
                .then(data => {
                    ...
                    if (!Array.isArray(data) || data.length === 0) {
                        document.getElementById("noFormsMessage").innerHTML = "No routing form data available.";
                        return;
                    }
