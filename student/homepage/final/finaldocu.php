...
function sendAdviserNotificationEmail($adviser_email, $adviser_name, $fullname, $title) {
    try {
        ...
        ";
        $mail->AltBody = "Dear {$adviser_name}, A new final defense document (Final Document) has been submitted by {$fullname} with the title '{$title}' and requires your review. Please login at: https://capstone.smccnasipit.edu.ph/trs/adviser/login.php";

        $mail->send();
        error_log("Adviser notification sent (Final Document) to: $adviser_email");
        return true;
    } catch (Exception $e) {
        error_log("Mailer Error in sendAdviserNotificationEmail (Final Document): " . $e->getMessage());
        return false;
    }
}
