...
// Get finaldocu_id from GET parameter
$finaldocu_id = isset($_GET['finaldocu_id']) ? intval($_GET['finaldocu_id']) : 0;
$student_id = $_SESSION['student_id'];

if ($finaldocu_id <= 0) {
    header('Content-Type: application/json');
    echo json_encode(['error' => 'Invalid finaldocu_id']);
    exit;
}

// First verify the student owns this finaldocu record
$stmt = $conn->prepare("SELECT 1 FROM finaldocuproposal_files WHERE student_id = ? AND finaldocu_id = ?");
$stmt->bind_param("si", $student_id, $finaldocu_id);
$stmt->execute();
