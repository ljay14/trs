...
function sendAdviserNotificationEmail($adviser_email, $adviser_name, $fullname, $title) {
    try {
        ...
        ";
        $mail->AltBody = "Dear {$adviser_name}, A new thesis document (Route 2) has been submitted by {$fullname} with the title '{$title}' and requires your review. Please login at: {$login_url}";
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delete_file'])) {
    ...
    $stmt = $conn->prepare("SELECT docuRoute2 FROM route2proposal_files WHERE student_id = ? AND docuRoute2 = ?");
    ...
    if ($stmt->num_rows > 0) {
        ...
        $deleteStmt = $conn->prepare("DELETE FROM route2proposal_files WHERE student_id = ? AND docuRoute2 = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        if (in_array($_FILES["docuRoute2"]["type"], $allowedTypes)) {
            ...
            if (move_uploaded_file($fileTmpPath, $newFilePath)) {
                ...
                $updateStmt = $conn->prepare("UPDATE route2proposal_files SET docuRoute2 = ? WHERE route2_id = ?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_SESSION['csrf_token'], $_POST['csrf_token']) && $_SESSION['csrf_token'] === $_POST['csrf_token']) {
    ...
    // Fetch the department, adviser and other details from the student's account
        ...
        if (isset($_FILES["docuRoute2"]) && $_FILES["docuRoute2"]["error"] == UPLOAD_ERR_OK) {
            ...
            if (in_array($_FILES["docuRoute2"]["type"], $allowedTypes)) {
                ...
                if ($count > 0) {
                    ...
                    $stmt = $conn->prepare("INSERT INTO route2proposal_files (student_id, docuRoute2, department, panel1_id, panel2_id, panel3_id, panel4_id, panel5_id, adviser_id, date_submitted, controlNo, fullname, group_number, title, school_year) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                    if ($stmt) {
                            ...
                            if ($adviser_email) {
                                if (isValidEmail($adviser_email)) {
                                    ...
                                    if ($emailSent) {
                                        ...
                                        error_log("Success: Notification email sent to adviser ($adviser_email) for Route 2 file upload.");
                                    } else {
                                        ...
                                        error_log("Error: Failed to send notification email to adviser ($adviser_email) for Route 2 file upload.");
                                    }
                                } else {
                                    echo "<script>alert('File uploaded successfully but adviser email address is invalid.'); window.location.href = 'route2.php';</script>";
                                    error_log("Error: Invalid adviser email address ($adviser_email) for Route 2 file upload.");
                                }
                            } else {
                                ...
                                error_log("Warning: No adviser email available for notification during Route 2 file upload.");
                            }
                        } else {
...
$stmt = $conn->prepare("SELECT panel1_id, panel2_id, panel3_id, panel4_id, panel5_id, adviser_id FROM route1proposal_files WHERE route1_id = ?");
...
$stmt = $conn->prepare("SELECT COUNT(*) FROM proposal_monitoring_form WHERE route1_id = ? AND adviser_id = ?");
...
<body>
    ...
    <form action="route2.php" method="POST" enctype="multipart/form-data" id="file-reupload-form" style="display: none;">
        ...
        <input type="file" name="docuRoute2" id="docuRoute2_reupload" accept=".pdf" required>
    </form>
    ...
    <script>
        ...
        function viewFile(filePath, student_id, route2_id) {
            ...
            // Load form data dynamically using student_id and route2_id
            fetch(`route2get_all_forms.php?student_id=${encodeURIComponent(student_id)}`)
                .then(res => res.json())
                .then(data => {
