        ...
        if (isset($_FILES["docuRoute2"]) && $_FILES["docuRoute2"]["error"] == UPLOAD_ERR_OK) {
            ...
            if (in_array($_FILES["docuRoute2"]["type"], $allowedTypes)) {
                ...
                if ($count > 0) {
                    ...
                    $stmt = $conn->prepare("INSERT INTO route2proposal_files (student_id, docuRoute2, department, panel1_id, panel2_id, panel3_id, panel4_id, adviser_id, date_submitted,controlNo, fullname, group_number, title, school_year) VALUES (?, ?, ?, ?, ?, ?, ?, ? ,? ,? ,? ,? , ?, ?)");
                    if ($stmt) {
