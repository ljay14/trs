...
// Get route2_id from GET parameter
$route2_id = isset($_GET['route2_id']) ? intval($_GET['route2_id']) : 0;
$student_id = $_SESSION['student_id'];

if ($route2_id <= 0) {
    header('Content-Type: application/json');
    echo json_encode(['error' => 'Invalid route2_id']);
    exit;
}

// First verify the student owns this route2 record
$stmt = $conn->prepare("SELECT 1 FROM route2proposal_files WHERE student_id = ? AND route2_id = ?");
$stmt->bind_param("si", $student_id, $route2_id);
$stmt->execute();
