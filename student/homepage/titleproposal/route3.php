...
function sendAdviserNotificationEmail($adviser_email, $adviser_name, $fullname, $title) {
    try {
        ...
        $mail->isHTML(true);
        $mail->Subject = 'New Thesis Document (Route 3) Submitted for Review';
        $mail->Body = "
            <div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 5px;'>
                ...
                <p>A new thesis document (Route 3) has been submitted and requires your review.</p>
        ...
        ";
        $mail->AltBody = "Dear {$adviser_name}, A new thesis document (Route 3) has been submitted by {$fullname} with the title '{$title}' and requires your review. Please login at: https://capstone.smccnasipit.edu.ph/trs/adviser/login.php";

        $mail->send();
        error_log("Adviser notification sent (Route 3) to: $adviser_email");
        return true;
    } catch (Exception $e) {
        error_log("Mailer Error in sendAdviserNotificationEmail (Route 3): " . $e->getMessage());
        return false;
    }
}
    ...
    if (!$department) {
        ...
        if (in_array($_FILES["docuRoute3"]["type"], $allowedTypes)) {
            ...
            if ($count > 0) {
                ...
                $stmt = $conn->prepare("INSERT INTO route3proposal_files (student_id, docuRoute3, department, panel1_id, panel2_id, panel3_id, panel4_id, panel5_id, adviser_id, date_submitted, controlNo, fullname, group_number, title, school_year) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                    ...
                    if ($adviser_email) {
                        if (isValidEmail($adviser_email)) {
                            ...
                            if ($emailSent) {
                                ...
                                error_log("Success: Notification email sent to adviser ($adviser_email) for Route 3 file upload.");
                            } else {
                                ...
                                error_log("Error: Failed to send notification email to adviser ($adviser_email) for Route 3 file upload.");
                            }
                        } else {
                            echo "<script>alert('File uploaded successfully but adviser email address is invalid.'); window.location.href = 'route3.php';</script>";
                            error_log("Error: Invalid adviser email address ($adviser_email) for Route 3 file upload.");
                        }
                    } else {
                        ...
                        error_log("Warning: No adviser email available for notification during Route 3 file upload.");
                    }
                } else {
            ...
            <div class="content" id="content-area">
                ...
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<!-- Debug: R3 Forms: $route3FormCount, Adviser Forms: $adviserFormCount, R2 Approved: $route2ApprovedCount, Minutes Source: $minutesSource -->";
