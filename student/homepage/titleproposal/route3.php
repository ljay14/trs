        ...
        if (isset($_FILES["docuRoute3"]) && $_FILES["docuRoute3"]["error"] == UPLOAD_ERR_OK) {
            ...
            if (in_array($_FILES["docuRoute3"]["type"], $allowedTypes)) {
                ...
                if ($count > 0) {
                    ...
                    $stmt = $conn->prepare("INSERT INTO route3proposal_files (student_id, docuRoute3, department, panel1_id, panel2_id, panel3_id, panel4_id, adviser_id, date_submitted) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
                    if ($stmt) {
    ...
    <div class="container">
        ...
        <div class="main-content">
            ...
            <div class="content" id="content-area">
                <?php
                $student_id = $_SESSION['student_id'];
                $stmt = $conn->prepare("SELECT docuRoute3, route3_id FROM route3proposal_files WHERE student_id = ?");
                $stmt->bind_param("s", $student_id);
                $stmt->execute();
