...
$stmt = $conn->prepare("SELECT student_id, route3_id FROM route3proposal_files 
...
if ($result->num_rows > 0) {
    ...
    if (!$route3_advised) {
        ...
        if ($route2Result && $route2Row = $route2Result->fetch_assoc()) {
            ...
            $route2AdviserCheck = $conn->prepare("SELECT COUNT(*) as count FROM proposal_monitoring_form 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['dateSubmitted'])) {
    ...
    // Prepare the query
    $stmt = $conn->prepare("INSERT INTO proposal_monitoring_form (panel_id, panel_name, date_submitted, chapter, feedback, paragraph_number, page_number, date_released, docuRoute3, route3_id, student_id, status, routeNumber) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
<body>
    ...
    $checkPanel1Query = "SELECT COUNT(*) as count FROM route3proposal_files WHERE panel1_id = ? OR panel2_id = ? OR panel3_id = ? OR panel4_id = ? OR panel5_id = ?";
