...
if (isset($data['id']) && isset($data['status']) && isset($_SESSION['panel_id'])) {
    ...
    $checkQuery = "SELECT panel_id FROM proposal_monitoring_form WHERE id = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        if ($row['panel_id'] != $panel_id) {
            echo json_encode(["success" => false, "message" => "You are not allowed to approve the status because you are not the assigned Panel and Adviser."]);
        } else {
            // Authorized - proceed with update
            $query = "UPDATE proposal_monitoring_form SET status = ? WHERE id = ? AND panel_id = ?";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("sii", $status, $id, $panel_id);
            if ($stmt->execute()) {
