...
if ($result->num_rows > 0) {
    ...
    $adviserCheck = $conn->prepare("SELECT COUNT(*) as count FROM final_monitoring_form WHERE route2_id = ? AND adviser_id IS NOT NULL");
    ...
    if (!$route2_reviewed) {
        ...
        $route1Check = $conn->prepare("SELECT r.route1_id FROM route1final_files r WHERE r.student_id = ?");
        ...
        if ($route1Result && $route1Row = $route1Result->fetch_assoc()) {
            ...
            $route1ApprovalCheck = $conn->prepare("SELECT COUNT(*) as count FROM final_monitoring_form 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['dateSubmitted'])) {
    ...
    // Prepare the query
    $stmt = $conn->prepare("INSERT INTO final_monitoring_form (panel_id, panel_name, date_submitted, chapter, feedback, paragraph_number, page_number, date_released, docuRoute2, route2_id, student_id, status, routeNumber) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
<body>
    ...
    $checkPanel1Query = "SELECT COUNT(*) as count FROM route2final_files WHERE panel1_id = ? OR panel2_id = ? OR panel3_id = ? OR panel4_id = ? OR panel5_id = ?";
    ...
    <?php endif; ?>
        ...
        <nav class="sidebar">
            ...
            <?php
                ...
                // For each file, check if adviser approved final route1
                if ($result->num_rows > 0) {
                    while($row = $result->fetch_assoc()) {
                        $student_id = $row['student_id'];
                        $route2_id = $row['route2_id'];
                        ...
                        // Only check final route1 approval - ignore route2 status
                        // Get route1_id
                        $get_route1 = $conn->prepare("SELECT route1_id FROM route1final_files WHERE student_id = ?");
                        ...
                        if ($route1_result && $route1_row = $route1_result->fetch_assoc()) {
                            ...
                            // Check if final route1 was approved
    ...
    <script>
        ...
        function viewFile(filePath, route2_id, student_id) {
            // First check if the adviser has approved route1 final
