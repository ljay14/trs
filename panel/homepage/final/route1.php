...
// Function to send email data to background process
function sendEmailInBackground($student_email, $student_name, $panel_name, $feedback_summary) {
    try {
        ...
        $email_data = [
            ...
            'is_final' => true // Flag to indicate this is a final document notification
        ];
        ...
        if (!file_put_contents($temp_file, json_encode($email_data))) {
            logEmailError("Failed to write email data to temporary file: $temp_file");
            unlink($temp_file);
            return false;
        }
...
// Function to send student notification
function sendStudentNotificationEmail($student_email, $student_name, $panel_name, $feedback_summary) {
    return sendEmailInBackground($student_email, $student_name, $panel_name, $feedback_summary);
}
...
.search-box {
    ...
    function viewFile(filePath, route1_id, student_id) {
        ...
        fetch(`check_adviser_route1.php?route1_id=${route1_id}`)
            .then(response => {
                if (!response.ok) {
            ...
            .catch(error => {
                ...
                // Do not allow viewing the file if there's an error checking adviser review status
...
<body>
    ...
    <?php endif; ?>
        ...
        <nav class="sidebar">
            ...
            // For each file, check if adviser has submitted ANY review form (not just approved ones)
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    $student_id = $row['student_id'];
                    $route1_id = $row['route1_id'];
