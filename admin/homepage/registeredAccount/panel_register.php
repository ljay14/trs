...
<head>
    ...
    <title>Panel Registration - Thesis Routing System</title>
    <style>
        :root {
            --primary: #002366;
...
<body>
                                    ...
                                    <tr id="row_<?= $row['school_id'] ?>">
                                        <form action="update_panel_inline.php" method="POST" id="form_<?= $row['school_id'] ?>">
                                            ...
                                            <td>
                                                ...
                                                <input type="email" name="email" value="<?= htmlspecialchars($row['email']) ?>" id="email_input_<?= $row['school_id'] ?>" style="display:none;">
                                            </td>
                                            <td>
    ...
    <script>
        ...
        document.getElementById('searchInput').addEventListener('keyup', function() {
            ...
            rows.forEach(row => {
                const nameCell = row.querySelector('td:nth-child(2)'); // Second column for fullname
                if (nameCell) {
                    // Get the text from the span element that contains the name
