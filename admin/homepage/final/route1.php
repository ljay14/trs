...
// Fetch school years
$schoolYears = [];
$schoolYearQuery = "SELECT DISTINCT school_year FROM route1final_files ORDER BY school_year DESC";
...
$sqlQuery = "SELECT 
    r2.docuRoute1 AS filepath, 
    r2.docuRoute1 AS filename, 
    r2.date_submitted,
    r2.controlNo,
    r2.group_number,
    r2.fullname,
...
if (!empty($selectedSemester)) {
    $sqlQuery .= " AND r2.student_id IN (SELECT student_id FROM student WHERE semester = ?)";
    $types .= "s";
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['selected_files'])) {
    ...
    if (empty($selectedFiles)) {
        ...
        foreach ($selectedFiles as $filePath) {
            ...
            $checkStmt = $conn->prepare("SELECT * FROM route1final_files WHERE docuRoute1 = ?");
            if ($checkStmt === false) {
            ...
            if ($fileExists) {
                ...
                $updatePanelStmt = $conn->prepare("UPDATE route1final_files 
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['update_assignments'])) {
    ...
    $updateStmt = $conn->prepare("UPDATE route1final_files 
        ...
        <?php if (!empty($files)): ?>
            <?php foreach ($files as $file): ?>
                <?php
                    ...
                    $department = htmlspecialchars($file['department'] ?? '', ENT_QUOTES);
